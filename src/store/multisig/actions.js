...
export async function saveWallet ({ commit, getters, rootGetters }, { multisigWallet, syncAcrossDevices = false }) { 
  if (!syncAcrossDevices) {
    ...
    if (!signerLocalWallet)
      throw new Error('Signer\'s xpub not found on this device!')
    
    let nominatedUploader = null

    for (const signerIndex in multisigWallet.signers) {
      const wallet = signerLocalWallets.find((localWallet) => {
        return localWallet.wallet.bch.xPubKey === multisigWallet.signers[signerIndex].xpub
      })
      if (wallet) { 
    ...
    if (!nominatedUploader || nominatedUploader?.hdKeys.hdPublicKey !== multisigWallet.signers[uploaderSignerId].xpub)
      throw new Error('Sync requires atleast 1 signer can sign on this device.')
