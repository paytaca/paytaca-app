      ...
      <q-card-section class="col-12 justify-center q-gutter-y-sm">
        ...
        <div class="flex column text-center q-gutter-y-xl" style="margin-top: 20px;">
          <div>
            ...
            <div class="q-pt-xs text-center text-capitalize" style="font-size: 13px;">Display Tx QR Code</div>
          </div>
          <div>
            <q-btn color="primary" class="button-default" :class="darkMode ? 'dark' : 'light'" round size="14px">
              <q-icon class="default-text-color"  size="24px" name="file_download" @click="handleDownloadPst"/>
            </q-btn>
            <div class="q-pt-xs text-center text-capitalize" style="font-size: 13px;">Download Tx File</div>
          </div>
        </div>
      </q-card-section>
      <q-card-actions>
...
const handleDownloadPst = () => {
  onDialogOK({ action: 'download-pst', pst: props.pst })
}
