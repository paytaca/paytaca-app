...
import { updateOrCreateKeypair } from 'src/wallet/ramp/chat'
  ...
  methods: {
    async loadChatIdentity () {
      ...
      vm.fetchChatIdentity(data.ref)
        ...
        .finally(() => {
          if (createChatIdentity) {
            vm.createChatIdentity(data)
          }
        })
    },
    async fetchChatIdentity (ref) {
    ...
    async createChatIdentity (data) {
      await this.buildChatIdentityPayload(data)
        .then(payload => {
