  ...
  methods: {
    ...
    async login () {
      ...
      try {
        ...
        this.$axios.get(otpUrl, { headers: { 'wallet-hash': this.wallet.walletHash } })
          .then((response) => {
            getWalletKeypair()
              .then((keypair) => {
                signMessage(keypair.privateKey, response.data.otp)
                  .then((signature) => {
                    const body = {
    ...
    async createRampUser () {
      ...
      getWalletKeypair()
        .then((keypair) => {
          signMessage(keypair.privateKey, 'PEER_CREATE', timestamp)
            .then((signature) => {
              const headers = {
