<template>
    <div class="q-mx-md q-mx-none q-mb-lg text-bow" :class="getDarkModeClass(darkMode)" :style="`height: ${minHeight}px;`">
...
export default {
  ...
  methods: {
    ...
    //   this.$router.push({ name: 'p2p-ads-create-form', query: { type: this.transactionType } })
    },
    onEditAd (id) {
      const vm = this
      vm.state = 'edit'
      vm.selectedAdId = id
      vm.pageName = 'ad-form-1'
      this.$router.push({ name: 'p2p-ads-edit-form', params: { ad: id } })
    },
    onDeleteAd (id) {
      const vm = this
