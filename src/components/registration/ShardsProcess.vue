<template>
  ...
  <template v-else>
    <headerNav title="Create Account" subtitle="Shards" :useEmitBack="true" :hasGradient="true" @back="$emit('back')"/>   
    <h5
      v-if="!fromWalletInfo"
      class="q-ma-none text-bow"
      :class="getDarkModeClass(darkMode)"
      style="padding: 90px 24px 0px;"
    >
      {{ $t('ShardsBackupPhase') }}
    </h5>
    ...
    <div style="padding: 0px 24px 24px">
        ...
        <div class="q-mt-lg text-bow" :class="getDarkModeClass(darkMode)">
          ...
          >
            ...
            <div class="flex flex-center q-mt-md q-mb-sm">
              <q-btn
                style="padding: 10px 15px 10px; border-radius: 10px;"                
      ...
      <q-btn
        ...
        :disable="!enableContinue"
        :label="$t('Continue')"
        class="q-mt-lg full-width button-default"
        @click="$emit('proceed-to-next-step')"
        style="border-radius: 10px; padding: 10px 0px 10px;"
      />
    </div>
