<template>
  <div id="app-container" class="debug-page sticky-header-container" :class="getDarkModeClass(darkMode)">
    ...
    <div class="q-pa-md q-mt-sm">
      ...
      <!-- Push Notification Re-registration -->
      <div class="q-mb-md" v-if="$q.platform.is.mobile">
        <q-card class="debug-card" :class="getDarkModeClass(darkMode)">
          <q-card-section>
            <div class="text-subtitle1 text-weight-medium q-mb-sm" :class="darkMode ? 'text-grey-4' : 'text-grey-8'">
  ...
  methods: {
    ...
    async forceReRegisterPush () {
      ...
      try {
        ...
        this.$q.notify({
          type: 'info',
          message: 'New token: ' + (newToken ? newToken.substring(0, 20) + '...' : 'null'),
          timeout: 3000
        })
